...
import { unwrapTrpcArray } from "@/src/lib/trpcData";
import {
  Smartphone,
  QrCode,
  Wifi,
...
export default function AdminClienteConexao() {
  const [, params] = useRoute<{ empresaId: string }>("/admin/cliente/:empresaId");
  const empresaId = params?.empresaId;
  const [, setLocation] = useLocation();

  const { data: empresasData } = trpc.admin.empresas.useQuery();
  const empresas = unwrapTrpcArray<any>(empresasData);
  const empresaIdNumber = empresaId ? Number(empresaId) : NaN;
  const empresaData = empresas.find((e: any) => e.id === empresaIdNumber);
          ...
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground ml-12">
                Gerenciando: <strong>{empresaData.nome}</strong>
              </p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <Card className="border border-border">
                ...
                <CardContent className="space-y-3">
                  <div>
                    ...
                    <p className="font-semibold">{empresaData.nome}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Número WhatsApp</p>
                    <p className="font-semibold">
                      {empresaData.whatsappNumero || "Não configurado"}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Licença Expira Em</p>
                    <p className="font-semibold">
                      {empresaData.licencaExpira
                        ? new Date(empresaData.licencaExpira).toLocaleDateString(
                            "pt-BR"
                          )
                        : "Sem data"}
                    </p>
              ...
              <Card className="border border-border bg-muted/50">
                ...
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <p>2. Abra o WhatsApp no seu telefone</p>
                  <p>3. Vá para Configurações → WhatsApp Web</p>
                  <p>4. Escaneie o código QR com sua câmera</p>
                  <p>5. A conexão será estabelecida automaticamente</p>
                </CardContent>
              </Card>
            </div>
          </div>
