...
export default function Admin() {
  ...
  const criarEmpresa = trpc.admin.criarEmpresa.useMutation({ onSuccess: () => refetch() });
  const toggleLicenca = trpc.admin.toggleLicenca.useMutation({ onSuccess: () => refetch() });
  ...
  const handleCreate = async () => {
    ...
    setShowForm(false);
    setForm({ nome: "", tipo: "pizzaria", whatsappNumero: "", emailUsuario: "", senhaUsuario: "", nomeUsuario: "" });
  };
          ...
          <Card className="border border-emerald-500/30 bg-emerald-500/5 mb-8">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Email de Login</label>
                  <Input
                    id="admin-usuario-email"
                    type="email"
                    placeholder="denis@pizzaria.com"
                    value={form.emailUsuario}
                    onChange={(e) => setForm({ ...form, emailUsuario: e.target.value })}
                    className="border-border"
                  />
                </div>
                <div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {criarEmpresa.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
                  Criar Empresa
                </Button>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        {empresa.ativo ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-500/10 text-emerald-600 text-xs rounded-full font-medium">
                            <CheckCircle2 className="w-3 h-3" />
                            Ativa
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-red-500/10 text-red-600 text-xs rounded-full font-medium">
                        ...
                        <div className="flex items-center gap-2 text-muted-foreground">
                          ...
                          <span>
                            {empresa.licencaExpira
                              ? `Expira: ${new Date(empresa.licencaExpira).toLocaleDateString("pt-BR")}`
                              : "Sem data"}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      {empresa.ativo ? (
                        <Button
                          ...
                          onClick={() =>
                            toggleLicenca.mutate({ empresaId: empresa.id, ativo: false })
                          }
                        >
                          Desativar
                        </Button>
